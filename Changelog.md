 ...
 * Optimizer: Minimal Yul optimizations needed to prevent "Stack too deep" errors (i.e. the stack allocation optimization and the ``UnusedPruner`` optimizer step) are now performed even when the user requests Yul optimizer to be disabled.
 ...
 * Standard JSON: Stack allocation optimization is now enabled by default.
